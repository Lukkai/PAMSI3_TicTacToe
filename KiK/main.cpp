...
START:
    ...
    sf::Texture texture;    texture.loadFromFile("resources\\rama.jpg");   //tekstura ramek (textures.com)
    sf::Texture tlo;        tlo.loadFromFile("resources\\back.jpg");        //tekstura tła (charrodelrey.com)
    sf::Texture wyjscie;    wyjscie.loadFromFile("resources\\exit.png");   //tekstura guzika exit (wikipedia.org)
    sf::Texture kolko;      kolko.loadFromFile("resources\\o.png");        //tekstura kółka (własna)
    sf::Texture krzyzyk;    krzyzyk.loadFromFile("resources\\x.png");      //tekstura krzyżyka (własna)
    sf::Texture puste;      puste.loadFromFile("resources\\-.png");        //pusty obraz .png stawiany w pustym polu (własny)
    ...
    sf::Texture text;       text.loadFromFile("resources\\wygrana.png"); //Tekst wyświetlany w komunikatach jest również teksturą (własną)
    ...
    czcionka.loadFromFile("resources\\Obstacle.ttf");   //Zastosowana czcionka: EFN-eukalipte
    ...
    while(opcje.isOpen()){  //gdy okno 'opcje' jest otwarte:
        ...
        text.loadFromFile("resources\\jaka.png");      //ustawienie tekstury tekstu (własna)
        tekst.setTexture(text);             //przekazanie jej do sprite'a
        opcje.draw(tekst);                  //wyrysowanie tekstu
        potw.loadFromFile("resources\\--.png");        //ustawienie tekstury minusa (własna)
        ye.setTexture(potw);                //przekazanie jej do sprite'a
        opcje.draw(ye);                     //wyrysowanie minusa
        potw.loadFromFile("resources\\+.png");         //ustawienie tekstury plusa (własna)
        nah.setTexture(potw);               //przekazanie jej do sprite'a
        opcje.draw(nah);                    //wyrysowanie plusa
        potw.loadFromFile("resources\\ok.png");        //ustawienie tekstury zatwierdzenia (własna)
        ...
        if(obszar.contains(Mysz)){              //jezeli wskaźnik znajduje się na tym obszarze
            potw.loadFromFile("resources\\--.png");        //wczytaj ponownie teksturę, gdyż mogła zostać nadpisana
            //przeskaluj sprite'a
        ...
        if(obszar.contains(Mysz)){                                  //analogiczne operacje
            potw.loadFromFile("resources\\+.png");
        ...
        if(obszar.contains(Mysz)){                                  //gdy wskaźnik na obszarze:
            potw.loadFromFile("resources\\ok2.png");                           //wczytaj alternatywną teksturę (kolor zielony)
    ...
    sf::Sprite PLANSZA[11][11];             //utworzenie tablicy sprite'ów (każdy to oddzielne pole)
    ...
    for(int i=0; i<plansza.wymiar; i++){
        ...
        if(czykomp&&!plansza.czykoniec()){  //gdy kolej AI i są miejsca na planszy
            plansza.choseMove();              //AI wykonuje ruch
    ...
    WYNIKI:
        if(wynik_an==10){                           //gdy zwycięża krzyżyk
            ...
            text.loadFromFile("resources\\przegrana.png");     //ustaw teksturę na informację o wygranej AI
            tekst.setTexture(text);                 //przekaż ją do sprite'u
            okno.draw(tekst);                       //wyrysuj sprite'a
            potw.loadFromFile("resources\\ye.png");            //ustaw teksturę potwierdzającą
            ye.setTexture(potw);                    //przekaż ją do sprite'u
            okno.draw(ye);                          //wyrysuj sprite'a
            potw.loadFromFile("resources\\nah.png");           //wczytaj teksturę odrzucającą
            ...
            if (Obszar.contains(Mysz)){             //gdy mysz nad obszarem
                potw.loadFromFile("resources\\ye2.png");       //zmień teksturę na zieloną
                ye.setTexture(potw);                //przekaż ją do sprite'u
                okno.draw(ye);                      //wyrysuj sprite'a
                if (sf::Mouse::isButtonPressed(sf::Mouse::Left)){
            ...
            if (Obszar.contains(Mysz)){             //gdy mysz nad obszarem
                potw.loadFromFile("resources\\nah2.png");      //zmień teksturę na czerwoną
                nah.setTexture(potw);               //przekaż ją do sprite'u
        ...
        if(wynik_an==-10){
            ...
            if (Obszar.contains(Mysz)){
                potw.loadFromFile("resources\\ye2.png");
                ye.setTexture(potw);
                okno.draw(ye);
                if (sf::Mouse::isButtonPressed(sf::Mouse::Left)){
                    plansza.wyzeruj();
        ...
        if(wynik_an==0&&plansza.czykoniec()){
            okno.draw(komunikat);
            text.loadFromFile("resources\\remis.png");
            tekst.setTexture(text);
            okno.draw(tekst);
